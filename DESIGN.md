...
- Respects `prefers-reduced-motion`. Because the motion here comes from eight
  ...
  - The Categories inspection processes (`src/components/category-inspection`,
    ...
    scene (`src/components/textiles-inspection`) is a realistic 3D model with no
    idle motion: scroll progress moves its camera toward each element and moves
    that element (the tag lifts, the swatches rise, the fabric tears), and it
    draws only when that pose, its size or its visibility changes. As in the
