...
- **Homepage depth**: the first page polished under the current palette. Glass assurance chips under the hero actions; a faint gold light behind the hero copy, desktop only, because below `lg` the copy crosses the photograph; gold-ink metric figures; a raised, rounded image stage in the services sequence; gold icon chips and figures on the audience panels; bright gold goods with gold-ink sign-off lines in the route drawing; and a raised closing CTA panel with a bright-to-deep gold rule along its top edge. After the 2026-09-16 review:
  - The hero's primary action keeps the gold hover pill (the navy dot floods it and the label slides out), now rendered as the link itself with `InteractiveHoverLink` instead of a button nested in a link that was focused twice. The sliding copy of the label is `aria-hidden`, so it is read once, and keyboard focus plays the same flood as hover. The other pages still nest the button until their own review.
  - The metric figures are rendered with their real values on the server. `AnimeCounter` counts up only when a figure starts off screen, reads its final value to screen readers, and uses tabular digits. The defect ceiling does not count up at all, because every figure on the way would claim a better rate than the target.
  - A "Skip to main content" link (`src/components/SkipLink.tsx`, mounted in RootLayout for every page) appears as a gold button on the first Tab and moves focus to `<main>`.
  - The route drawing names its three stations under the stations themselves, placed from the model (`stationAnchors`). A name lights with its station when a stage is hovered, focused or pinned. It replaces the old "Source / Global" and "Destination / North America" caption.
  - On phones the drawing pins beneath the header while its stages scroll past, which is why its section is `overflow-clip` rather than `overflow-hidden`.
  - The services sequence spends 32vh of scroll per step instead of 60.
...
- **Categories depth**: the third page polished. The hero stays text-led, so it is not a third copy of the photographic heroes, and from `lg` it gains a raised index of the five manufacturing portfolios: each row shows the lead time and opens that category in the portfolio section, scrolling it into view. The proof figures sit on a raised panel overlapping the hero, with each label ahead of its figure in the markup. The portfolio photograph is a raised stage with nothing laid over it (a number-and-name caption was removed because it only repeated the selected tab and the label beside the photo); the specification sits in one ivory block, and inspection protocols carry gold check chips. The four governance checkpoints are raised white cards on the ivory band with gold numbered badges, and the page closes on the shared raised CTA panel. No photographs were added or moved, so the CDN-served category photographs appear only where they already did. Each category with an inspection model (so far Precision Hardware, Packaging & Retail and Textiles & Materials) has a "Photograph / Inspection process" switch under its photograph, offered only where WebGL 2 runs well. Inspection process replaces the whole panel with the inspection view: a hidden-line model in the photograph's 4:3 frame, half the panel's width and never taller than the screen allows, pinned beneath the header beside a process panel (the category label, an Inspection process heading, the step list on a gold progress rail, and the current step's caption). The title, description, specification and inquiry link step aside until Photograph is chosen again, and switching views scrolls the panel into place, so the process starts at its first step. Scrolling then moves through the category's three protocols: the model turns toward each inspection point, that step's gold effect plays out with the scroll, its numbered point comes forward and its caption crossfades in, and any step can be chosen from the list. On phones the step list becomes three step bars under the drawing. Leaving the category returns the panel to the photograph. The Packaging model is rebuilt with img2threejs from the owner's illustration (used as a modelling reference only, never published, because it carries baked-in text and invented dimension figures), and the drawings are labelled "Illustrative drawing, not to scale" because the Packaging carton is drawn smaller than in the illustration so it fits over the tray. Textiles & Materials is the exception to the line drawings, at the owner's request: its Inspection process shows one realistic 3D scene instead (a three-quarter perspective camera, soft shadows on the figure's own off-white ground, woven fabric, kraft board and printed card materials), with no numbered points; the step list and captions name each element, and its note reads "Illustrative 3D model, not to scale." The scene is one deep indigo fabric roll on a kraft tube with a layered end face, keeping the tube proportions fitted to a roll in the category's warehouse photograph (a modelling reference only), and one continuous sheet unrolled from it that drapes forward onto the ground. The three inspection elements are geometry on that sheet: a printed certification tag tied through its edge near the roll, five pinked colour swatches in spectrum order, and the stretch of its near edge that the tear test tears. Each is a named group with a hit area, ready for hover and click. Scrolling moves the camera from the whole roll to a closer three-quarter view of each element in turn, and that element moves during its own step: the tag lifts on its eyelet end, the swatches rise and tilt toward the viewer one after another, and the fabric tears: the sheet starts whole, then a tear runs in from its edge and widens as its sides pull apart, loose threads grow along the torn edges, and threads left across the gap snap back as it opens. The camera holds still on each element while it moves, and each movement is complete by its step's middle, so the held pose under reduced motion shows it. Home & Utility has a scene of the same kind (`src/components/home-inspection`, `src/lib/home-inspection`): a fluted celadon pitcher standing in a steel test bath, with a borosilicate beaker of food simulant beside it. Each of its protocols is carried out on the piece: the beaker lifts, tips over the mouth and pours until the simulant stands at the test line, where it stays; a coupon cut from the wall lifts out and turns so the glaze over the ceramic body reads in section, then goes back; and the bath fills and ripples around the piece. Both realistic scenes share one stage (`src/components/inspection-3d/stage.ts`): the renderer, the studio light and shadows sized to the model, and the camera that moves from the whole model to each area and holds there. The pitcher's scene lights its steel and glass from a studio environment as well, since a metal with nothing to reflect renders black. Seasonal & Promotional is the third scene of this kind (`src/components/seasonal-inspection`, `src/lib/seasonal-inspection`): a kraft shipping carton packed with retail gift boxes, printed in the site's ivory, gold and navy with no brand, beside a timber pallet. Its protocols run forward in the page's order: the packed carton is lifted onto the pallet and ships from there; the front row of gift boxes is drawn out as the AQL sample, one lid hinged open on its tinplate tin, then put back; and the flaps fold in, tape runs along the seam and the dispatch label lands on the top, where they stay.
...
- Respects `prefers-reduced-motion`. Because the motion here comes from eight
  ...
  - `motion/react` scroll reveals and hero entrances: gated by `useReducedMotion()`
    ...
    explicitly. A motion `initial={{ opacity: 0 }}` is written into the server
    HTML, so anything using it stays invisible until JavaScript runs, and forever
    if it doesn't. The homepage no longer does this. Its hero entrance is CSS
    (`.hero-rise` and `.hero-photo` in `globals.css`, switched off in the
    reduced-motion block), and its scroll reveals use `Reveal`
    (`src/components/ui/reveal.tsx`), which renders content visible on the server
    and hides only what is still below the viewport once the page is
    interactive. Reveal skips the animation under reduced motion. The other pages
    still use the old pattern until their own review.
  ...
  - The Categories inspection processes (`src/components/category-inspection`,
    ...
    `src/components/housing-inspection`, `src/components/packaging-inspection`
    and `src/components/textiles-inspection`): a category's figure switched to
    ...
    corner, compression arrows and a barcode scan on the display. The Textiles,
    Home & Utility and Seasonal scenes (`src/components/textiles-inspection`,
    `src/components/home-inspection` and `src/components/seasonal-inspection`,
    on the shared stage in `src/components/inspection-3d`) are realistic 3D
    models with no idle motion: scroll progress moves the camera toward each
    element and moves that element (the tag lifts, the swatches rise, the fabric
    tears; the beaker pours, the coupon lifts out of the wall, the bath fills;
    the carton goes on the pallet, the sample comes out, the carton is taped and
    labelled), and each draws only when that pose, its size or its visibility
    changes. As in the
    ...
    photograph and releases the WebGL context. All the drawings share one geometry
