...
- Respects `prefers-reduced-motion`. Because the motion here comes from eight
  ...
  - The Categories inspection processes (`src/components/category-inspection`,
    ...
    and Home & Utility scenes (`src/components/textiles-inspection` and
    `src/components/home-inspection`, on the shared stage in
    `src/components/inspection-3d`) are realistic 3D models with no idle motion:
    scroll progress moves the camera toward each element and moves that element
    (the tag lifts, the swatches rise, the fabric tears; the beaker pours, the
    coupon lifts out of the wall, the bath fills), and each draws only when that
    pose, its size or its visibility changes. As in the
